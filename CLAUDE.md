...
- **1999–2000 source-data gap.** SBIR.gov's export omits title *and* abstract for 28% of 1999 records and 84% of 2000 records — **6,118 of the 6,527 rows dropped by `build_artifacts.py` (94%)** come from those two years, vs <1% in every neighbour. The 1999 Phase II pool collapses from 1,313 rows in the raw CSV to 74 in the artifact, and DoD (the largest SBIR agency, 659 Phase II awards in 1998) vanishes from that year entirely. Since conversions land 1–2 years out (the 1995 cohort matches 14/739/323/65 across 1995–98), this craters the **1997–2000** cohorts — 1998 shows 5.3% against a ~35% plateau. A titleless Phase II row can never be fuzzy-matched, so this is unfixable downstream; `contract_end_date`/`proposal_award_date` are near-empty for the era too. `LOW_COVERAGE_YEARS = {1999, 2000}` in `conversion.py`, with `low_coverage_cohorts()` widening it to 1997–2000. `find_conversions(exclude_low_coverage=True)` drops them from the headline rate (34.7% → 36.4%); they are always flagged via `by_year["reliable"]` and drawn as a separate "Incomplete source data" chart series regardless. **`build_artifacts.py` recomputes the detection each build (>10% drop rate) and warns if it disagrees with the constant, so it cannot silently go stale.**
